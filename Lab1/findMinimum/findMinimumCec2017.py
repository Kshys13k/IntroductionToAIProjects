...
def gradientDescent(func, args, learningRate, maxIteration):
    for i in range(0, maxIteration):
        gradTable=numericGradient(func, args)
        for i in range(len(args[0])):
            args[0][i]=args[0][i]-learningRate*gradTable[i]
    return args, func(args)

def gradientDescentVerbose(func, args, learningRate, maxIteration):
    for i in range(0, maxIteration):
        gradTable=numericGradient(func, args)
        for i in range(len(args[0])):
            args[0][i]=args[0][i]-learningRate*gradTable[i]
...
def gradientDescentPlot(func, args, learningRate, maxIteration):
    MAX_X = 100
    PLOT_STEP = 1
