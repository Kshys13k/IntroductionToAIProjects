...
class FindMinimumBooth:
    def BoothFunction(self,x,y):
        return (x+2*y-7)**2+(2*x+y-5)**2
    def BoothFunctionXDerivative(self,x,y):
        return 10*x+8*y-34
    def BoothFunctionYDerivative(self,x, y):
        return 8*x+10*y-38
    ...
    def gradientDescent(self,X,Y,learningRate, maxIteration, epsilon):
        for i in range (0,maxIteration):
            print(str(X) + " " + str(Y) + " " + str(self.BoothFunction(X, Y)))
            X=X-(learningRate*self.BoothFunctionXDerivative(X,Y))
            Y=Y-(learningRate*self.BoothFunctionYDerivative(X,Y))
            if(self.BoothFunction(X,Y)<epsilon):
                maxIteration=i
                break
        return X, Y, self.BoothFunction(X,Y), maxIteration
    ...
    def gradientDescentPlot(self,X,Y,learningRate, maxIteration, epsilon):
        #creating plot:
        MAX_X = 10
        PLOT_STEP = 0.1
